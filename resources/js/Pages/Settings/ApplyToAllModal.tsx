...
interface CopyModalProps extends ModalProps {
	...
	const resetModal = () => { setSourceTerminalId(""); setTargetTerminalId(""); setSelectedOption("Network"); setSelectedTargetBranch(""); setExpandedTab(null); setCurrentStep(1); };
  ...
  // Notify parent when both terminals are selected
	useEffect(() => {
    ...
    ? [
        <div key="footer" className="flex gap-2 justify-end">
          ...
          <button
            onClick={() => {
              if (!sourceTerminal) {
                alert("Please select a source terminal.");
                return;
              }
		...
		{data?.length ? (
			...
			<div className="flex justify-center mb-6">
				...
				{["Network", "Branch"].map((option) => (
					...
					className={`w-70 py-2 text-sm font-medium rounded-lg transition-all duration-200 ${
						...
						? "bg-[var(--color-secondary-active)] text-white shadow"
			...
			{selectedOption === "Network" && (
				...
				<div className="w-full max-w-md space-y-3">
					...
					<select
					  className="w-full border border-gray-300 text-gray-800 rounded-lg h-10 px-3 bg-white shadow-sm
             ...
             transition duration-200 ease-in-out"
					...
					>
					<option value="">Select a source terminal</option>
					{data.map((terminal: any, index: number) => (
			...
			{selectedOption === "Branch" && (
				...
				<div className="w-full max-w-md space-y-3">
					...
					<select
					className="w-full border border-gray-300 text-gray-800 rounded-lg h-10 px-3 focus:outline-none focus:ring-2 focus:ring-[var(--primary)] focus:border-[var(--primary)] transition"
					value={sourceTerminalId}
					onChange={(e) => {
						setSourceTerminalId(e.target.value);
					}}
